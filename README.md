...
1. Installs or upgrades `inkbox>=0.7.1,<1.0.0` and `aiohttp>=3.9` in the Hermes Python environment when needed.
...
```bash
/path/to/hermes/venv/bin/python3 -m pip install 'inkbox>=0.7.1,<1.0.0' 'aiohttp>=3.9'
```

When `uv` is available, the wizard prefers:

```bash
uv pip install --python /path/to/hermes/venv/bin/python3 'inkbox>=0.7.1,<1.0.0' 'aiohttp>=3.9'
```
...
outbound-only compatibility aliases. The plugin requires Inkbox SDK 0.7.1 or
newer.
